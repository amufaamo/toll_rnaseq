...
# --- UI Definition ---
ui <- page_navbar(
    title = "EasyRNA-Seq v2.0",
    id    = "main_nav",
    window_title = "EasyRNA-Seq",
    theme = bs_theme(
        bootswatch = "flatly",
        version    = 5
    ),
    ...
    # Docker status + nav spacer on the right
    nav_spacer(),
    nav_item(
        tags$span(class = "navbar-text small",
                  "Docker: ", textOutput("docker_status", inline = TRUE))
    ),
    ...
    nav_menu(
        ...
        # Tab 1: Reference Prep
        nav_panel(
            title = "1. Reference Prep",
            layout_sidebar(
                sidebar = sidebar(
                    width = 340,
                    card(
                        card_header("Index Settings"),
                        radioButtons("index_type", "Index Type:",
                                     choices = c("Salmon (Transcriptome)" = "salmon", "STAR (Genome)" = "star"),
                                     inline = TRUE),
                        numericInput("ref_threads", "Threads", value = 4, min = 1, max = 32),
                        radioButtons("ref_mode", "Source:",
                                     choices = c("Local File" = "local", "URL Download" = "url", "Upload" = "upload"),
                                     inline = TRUE)
                    ),
                    card(
                        card_header("File Selection"),
                        conditionalPanel(
                        ...
                        conditionalPanel(
                            ...
                            shinyDirButton("download_dest_dir", "Download Destination", "Select folder"),
                            textOutput("download_dest_display")
                        ),
                        conditionalPanel(
                            condition = "input.ref_mode == 'upload'",
                            fileInput("upload_fasta", "Upload Fasta", accept = c(".fa", ".fasta", ".fna", ".gz")),
                            fileInput("upload_gtf",   "Upload GTF (Optional)", accept = c(".gtf", ".gff", ".gff3", ".gz")),
                            shinyDirButton("upload_dest_dir", "Save To", "Folder to save uploaded files"),
                    ...
                    card(
                        ...
                        actionButton("btn_make_index", "Build Index", class = "btn-primary w-100", icon = icon("dna")),
                        br(), br(),
        ...
        nav_panel(
            ...
            layout_sidebar(
                sidebar = sidebar(
                    ...
                    card(
                        card_header("Method"),
                        radioButtons("quant_method", NULL,
                                     choices = c("Salmon", "STAR + featureCounts"), inline = TRUE)
                    ),
                    card(
                        card_header("Inputs"),
                        h6("FASTQ Directory"),
                        shinyDirButton("fastq_dir", "Select FASTQ Folder", "Select folder with FASTQ files"),
                    ...
                    card(
                        ...
                        checkboxInput("t2_run_multiqc", "MultiQC Report", value = TRUE),
                        hr(),
                        h6("Output Directory"),
                    ...
                    card(
                        card_header("Detected Samples"),
                        tableOutput("sample_table"),
                        actionButton("btn_run_quant", "Start Analysis", class = "btn-success w-100", icon = icon("play")),
                ...
                layout_columns(
                    ...
                    card(card_header("Log Output"),  verbatimTextOutput("log_quant"), height = "400px"),
                    card(card_header("Results"),      uiOutput("results_ui"))
                )
            )
        ),

        # Tab 3: De novo Assembly
        nav_panel(
            ...
            layout_sidebar(
                sidebar = sidebar(
                    ...
                    card(
                        ...
                        div(class = "alert alert-warning mb-0",
                            icon("triangle-exclamation"), " Requires 32 GB+ RAM")
                    ),
                    card(
                        card_header("Inputs"),
                        h6("FASTQ Directory"),
                    ...
                    card(
                        ...
                        conditionalPanel(
                            ...
                            selectInput("busco_lineage", "BUSCO Lineage",
                                        choices  = c("eukaryota_odb10", "vertebrata_odb10", "mammalia_odb10",
                                                     "fungi_odb10", "bacteria_odb10", "metazoa_odb10", "archaea_odb10"),
                                        selected = "eukaryota_odb10")
                        ),
                        checkboxInput("t3_run_multiqc", "MultiQC Report", value = TRUE)
                    ),
                    card(
                        card_header("Detected Samples"),
                        tableOutput("trinity_sample_table"),
                        actionButton("btn_run_trinity_pipeline", "Start De novo Pipeline",
                                     class = "btn-danger w-100", icon = icon("cogs")),
                ...
                layout_columns(
                    ...
                    card(card_header("Log Output"), verbatimTextOutput("log_trinity"), height = "400px")
                )
            )
        )
    ),
    ...
    nav_menu(
        ...
        icon  = icon("chart-bar"),
        nav_panel("1. Data Input",        dataUploadMetadataUI("dataTab")),
        nav_panel("2. Filtering",         filteringUI("filterTab")),
        nav_panel("3. Processing",        processingUI("procTab")),
        nav_panel("4. Dimension Reduction", dimensionReductionUI("dimRedTab")),
        nav_panel("5. DEG Analysis",      degAnalysisUI("degTab")),
        nav_panel("6. GSEA",              gseaUI("gseaTab")),
        nav_panel("7. GO Enrichment",     goEnrichmentIntegratedUI("go_module")),
        nav_panel("8. Time Series",       timeseriesAnalysisUI("timeseriesTab"))
    )
)
