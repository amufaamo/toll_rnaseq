    ...
    observeEvent(data_processing_trigger(), {
      ...
      tryCatch({
        ...
        if (inputType == "individual") {
          ...
          initial_sample_names <- make.unique(trim_common_strings(names_no_ext))
          
          # --- Subtask 1: Read only Geneid column to find common IDs and perform conversion ---
          message("[INDIVIDUAL_PROCESS] Stage 1: Reading only Geneids to find common and convert...")
          ...
          first_file_ids_for_detect <- geneid_lists[[1]]
          detected_keytype <- detect_gene_id_type(first_file_ids_for_detect)
          ...
          valid_converted_geneids_map <- data.table(
            OriginalGeneid = all_original_geneids[!conversion_res$failed_indices],
            Geneid = conversion_res$processed_ids[!conversion_res$failed_indices]
          )
          valid_converted_geneids_map <- valid_converted_geneids_map[!duplicated(Geneid), ] # Ensure unique converted IDs
          
          if(nrow(valid_converted_geneids_map) == 0) stop("ID変換可能な遺伝子がありませんでした。")
          
          # --- Subtask 2 & 3: Initialize merged_dt and iteratively merge counts using reference update ---
          message("[INDIVIDUAL_PROCESS] Stage 2: Initializing and merging count data (Memory Optimized)...")
          
          # Prepare the base table with unique Entrez IDs
          unique_entrez_ids <- unique(valid_converted_geneids_map$Geneid)
          merged_dt <- data.table(Geneid = unique_entrez_ids)
          setkey(merged_dt, Geneid)
          
          # Initialize Gene Lengths placeholder
          gene_lengths_dt <- NULL
          
          # Loop through all files to read and update merged_dt by reference
          for (i in seq_along(file_paths)) {
            # Read file (Geneid and Count)
            # For the first file, also read Length
            cols_to_read <- if (i == 1) c(1, 6, 7) else c(1, 7)
            col_names_read <- if (i == 1) c("OriginalGeneid", "Length", "Count") else c("OriginalGeneid", "Count")
            
            current_file_dt <- fread(file_paths[i], header = TRUE, sep = "\t", stringsAsFactors = FALSE, select = cols_to_read, na.strings = c("NA", "NaN", ""))
            setnames(current_file_dt, col_names_read)
            
            # Filter and Map IDs
            # Inner join with map ensures we only keep common/valid IDs
            current_file_dt_mapped <- merge(current_file_dt, valid_converted_geneids_map, by = "OriginalGeneid", all.x = FALSE, all.y = FALSE)
            
            # Aggregate counts (and Length for first file) by Entrez Geneid
            if (i == 1) {
              # For first file: capture Length and Counts
              agg_dt <- current_file_dt_mapped[, .(Length = first(Length), Count = sum(Count, na.rm = TRUE)), by = Geneid]
              
              # Save gene lengths
              rv$gene_lengths <- as.data.frame(agg_dt[, .(Geneid, Length)])
              
              # Update merged_dt with counts
              merged_dt[agg_dt, (initial_sample_names[i]) := i.Count, on = "Geneid"]
              
            } else {
              # For subsequent files: capture Counts only
              agg_dt <- current_file_dt_mapped[, .(Count = sum(Count, na.rm = TRUE)), by = Geneid]
              
              # Update merged_dt with counts by reference
              merged_dt[agg_dt, (initial_sample_names[i]) := i.Count, on = "Geneid"]
            }
            
            # Explicitly free memory
            rm(current_file_dt, current_file_dt_mapped, agg_dt)
            if (i %% 5 == 0) gc() 
          }
          
          # Fill NA values with 0 (in-place)
