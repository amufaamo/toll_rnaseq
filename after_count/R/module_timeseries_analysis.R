...
timeseriesAnalysisServer <- function(id, rv) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(input$load_active_samples, {
      ...
      shiny::validate(shiny::need(nrow(active_meta) > 0, "アクティブなサンプルがありません。「データ入力」タブでサンプルを選択してください。"))
    ...
    observeEvent(input$run_analysis, {
      ...
      # バリデーション
      shiny::validate(
        shiny::need(all(c("Time", "Replicate", "Group") %in% colnames(edesign_from_table)), "Time, Replicate, Group列が必要です。"),
        shiny::need(is.numeric(edesign_from_table$Time), "Time列は数値である必要があります。"),
        shiny::need(is.numeric(edesign_from_table$Replicate), "Replicate列は数値である必要があります。")
      )
      ...
      shiny::withProgress(message = 'maSigPro解析を実行中...', value = 0, {
        ...
        shiny::validate(shiny::need(all(sample_order %in% colnames(counts_filtered)), "edesignのサンプル名がカウントデータに見つかりません。"))
        ...
        shiny::validate(shiny::need(!is.null(design), "デザイン行列の作成に失敗しました。"))
        ...
        shiny::validate(shiny::need(!is.null(fit), "p.vectorの実行に失敗しました。"))
        ...
        shiny::validate(shiny::need(!is.null(tstep), "T.fitの実行に失敗しました。"))
        ...
        shiny::validate(shiny::need(!is.null(sigs), "有意遺伝子の抽出に失敗しました。"))
    ...
    observeEvent(input$plot_genes, {
      ...
      shiny::validate(
        shiny::need(!is.null(sig_genes_data), "選択された群の有意遺伝子データがありません。"),
        shiny::need(nrow(sig_genes_data) > 0, "選択された群に有意遺伝子がありません。"),
        shiny::need(nrow(sig_genes_data) >= input$k_clusters, paste("クラスター数(k)は遺伝子数(", nrow(sig_genes_data), ")以下にしてください。"))
      )
