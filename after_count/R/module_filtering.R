  ...
  sidebarLayout(
    sidebarPanel(
      ...
      div(id = ns("filtering_params"),
          h4(icon("filter"), "Filtering Parameters"),
          tags$div(class = "alert alert-info", 
                   icon("info-circle"), " edgeRの `filterByExpr` 関数を用いて低発現遺伝子を除外します。サンプルの「グループ」情報は自動的に考慮されます。"),
          numericInput(ns("min_count"), "Minimum Count (min.count)", value = 10, min = 0, step = 1),
          helpText(icon("question-circle"), " 遺伝子が発現しているとみなされる最小カウント数"),
          numericInput(ns("min_total_count"), "Minimum Total Count (min.total.count)", value = 15, min = 0, step = 1),
          helpText(icon("question-circle"), " 全サンプルにおける合計カウント数の最小値 (極端に発現が低い遺伝子を除外)"),
...
filteringServer <- function(id, rv) {
  moduleServer(id, function(input, output, session) {
    ...
    filtering_results <- reactive({
      ...
      shiny::validate(shiny::need(n_genes_before > 0, "Count data is not available."))
      ...
      if (input$perform_filtering == "no") {
        ...
        keep <- tryCatch({
          ...
          showNotification(paste("フィルタリング処理中にエラーが発生しました:", e$message), type="error", duration = 10)
          return(NULL)
        })
    ...
    output$filterPlot <- renderPlot({
      ...
      shiny::validate(shiny::need(length(keep) == nrow(logcpm_unfiltered), "Mismatch between filtering results and data."))
