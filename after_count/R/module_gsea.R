    ...
    sidebarLayout(
      sidebarPanel(
        ...
        radioButtons(ns("gsea_rank_metric"), "ランキング指標 (DEG結果より):",
          choices = c(
            "符号付き -log10(PValue)" = "signed_p",
            "logFC x (-log10(PValue))" = "logFC_x_signed_p",
            "log2 Fold Change" = "logFC",
            "符号付き 検定統計量 (F)" = "stat"
          ),
          selected = "logFC_x_signed_p"
        ), # デフォルトを logFC_x_signed_p に変更
      ...
      mainPanel(
        ...
        numericInput(ns("gseaPlotBaseSize"), "文字の大きさを変更 (Base Size):", value = 15, min = 1, step = 1),
        numericInput(ns("gseaPlotLineWidth"), "線の太さを変更:", value = 2.0, min = 0.1, step = 0.1),
        br(),
        withSpinner(plotOutput(ns("enrichmentPlot"), height = "500px"), type = 6),
        br(),
        downloadButton(ns("downloadPlotPDF"), "プロットをPDFで保存 (.pdf)", icon = icon("file-pdf"))
      )
    )
  )
}
  ...
  moduleServer(id, function(input, output, session) {
    ...
    gsea_results_reactive <- eventReactive(input$runGSEA, {
      ...
      if (metric_type == "logFC") {
        shiny::validate(shiny::need("logFC" %in% colnames(deg_table_symbol), "logFC列なし"))
        ranks_df <- deg_table_symbol[, c("GeneSymbol", "logFC"), drop = FALSE]
        colnames(ranks_df)[2] <- "rank_val"
      } else if (metric_type == "signed_p") {
        shiny::validate(shiny::need(all(c("PValue", "logFC") %in% colnames(deg_table_symbol)), "PValueまたはlogFC列なし"))
        pvals <- deg_table_symbol$PValue
        ...
        shiny::validate(shiny::need(!is.null(f_stat_col), "検定統計量列 (FまたはLR) がDEG結果にありません。"))
      ...
      shiny::validate(shiny::need(nrow(ranks_df) > 0, "有効なランキング指標を持つ遺伝子なし(NA除去後)。"))
      if (has_duplicates_symbol) {
        ranks_agg <- aggregate(rank_val ~ GeneSymbol, data = ranks_df, FUN = mean)
      } else {
        ranks_agg <- ranks_df
      }
      ranks <- setNames(ranks_agg$rank_val, ranks_agg$GeneSymbol)
      ranks <- sort(ranks, decreasing = TRUE)
      shiny::validate(shiny::need(length(ranks) > 0, "最終ランキングメトリクス作成失敗。"))
      ...
      if (input$gsea_selection_method == "category") {
        ...
        shiny::validate(shiny::need(
          !is.null(m_df_category) && nrow(m_df_category) > 0,
          paste("カテゴリ指定で遺伝子セットを取得できませんでした:", input$gsea_collection_category)
        ))
        shiny::validate(shiny::need("gene_symbol" %in% colnames(m_df_category), "msigdbr(category)結果に 'gene_symbol' 列なし。"))
        ...
        shiny::validate(shiny::need(
          !is.null(all_gs_data) && nrow(all_gs_data) > 0,
      ...
      shiny::validate(shiny::need(length(pathways) > 0, "解析対象のパスウェイがありませんでした。遺伝子セットの指定を確認してください。"))
      ...
      shiny::validate(shiny::need(
        !is.null(fgsea_res) && nrow(fgsea_res) > 0,
    ...
    enrichment_plot_obj <- reactive({
      ...
      shiny::validate(shiny::need(length(selected_row_indices) == 1, "結果テーブルから1行選択してください。"))
      selected_pathway_name <- gsea_output$results_table$pathway[selected_row_indices]
      shiny::validate(shiny::need(
        selected_pathway_name %in% names(gsea_output$pathways),
        paste0("選択されたパスウェイ '", selected_pathway_name, "' が遺伝子セットリストに見つかりません。")
      ))
      plot_obj <- fgsea::plotEnrichment(
        gsea_output$pathways[[selected_pathway_name]],
        gsea_output$ranks
      ) +
        labs(title = selected_pathway_name) +
        theme_bw(base_size = input$gseaPlotBaseSize)

      # 線の太さを変更
      if (length(plot_obj$layers) >= 1) {
        plot_obj$layers[[1]]$aes_params$linewidth <- input$gseaPlotLineWidth
        plot_obj$layers[[1]]$aes_params$size <- input$gseaPlotLineWidth # 古いggplot2への互換性のため
      }

      list(plot = plot_obj, name = selected_pathway_name)
    })

    output$enrichmentPlot <- renderPlot({
      plot_data <- enrichment_plot_obj()
    ...
    output$downloadPlotPDF <- downloadHandler(
      filename = function() {
        tryCatch(
          {
            plot_data <- enrichment_plot_obj()
            safe_name <- gsub("[^A-Za-z0-9_.-]", "_", plot_data$name)
            if (nchar(safe_name) > 50) safe_name <- substr(safe_name, 1, 50)
            paste0("GSEA_Plot_", safe_name, "_", Sys.Date(), ".pdf")
          },
          error = function(e) {
            paste0("GSEA_Plot_", Sys.Date(), ".pdf")
          }
        )
      },
    ...
    output$downloadGSEAResults <- downloadHandler(
      ...
      content = function(file) {
        ...
        # メタデータをコメント行として先頭に追加
        comparison_str <- if (!is.null(rv$deg_results) && !is.null(rv$deg_results$comparison)) {
          paste(rv$deg_results$comparison, collapse = " vs ")
        } else { "N/A" }

        collection_str <- if (input$gsea_selection_method == "category") {
          input$gsea_collection_category
        } else {
          input$gsea_search_pathway_name
        }

        comment_lines <- c(
          paste0("# Analysis Date: ", Sys.time()),
          paste0("# Species: ", input$gsea_species),
          paste0("# Gene Set Selection: ", input$gsea_selection_method),
          paste0("# Gene Set / Collection: ", collection_str),
          paste0("# Ranking Metric: ", input$gsea_rank_metric),
          paste0("# Min Gene Set Size: ", input$gsea_min_size),
          paste0("# Max Gene Set Size: ", input$gsea_max_size),
          paste0("# DEG Comparison: ", comparison_str),
          paste0("# DEG Method: ", rv$deg_results$analysis_method %||% "N/A"),
          paste0("# DEG Significance Metric: ", rv$deg_results$metric_at_run %||% "N/A"),
          paste0("# DEG Significance Threshold: ", rv$deg_results$sig_threshold_at_run %||% "N/A"),
          paste0("# DEG LogFC Threshold: ", rv$deg_results$logfc_threshold_at_run %||% "N/A"),
          ""
        )
        writeLines(comment_lines, con = file, useBytes = TRUE)
        write.table(gsea_output_dl$results_table, file, row.names = FALSE, quote = TRUE,
                     sep = ",", append = TRUE)
      }
    )
