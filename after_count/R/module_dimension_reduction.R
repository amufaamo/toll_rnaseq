...
dimensionReductionUI <- function(id) {
  ...
  fluidPage(
    ...
    hr(),
    fluidRow(
        column(4, checkboxInput(ns("use_batch"), "バッチ補正を適用して表示 (Batch Correction)", value = FALSE)),
        column(8, conditionalPanel(condition = paste0("input['", ns("use_batch"), "'] == true"), selectInput(ns("batch_col"), "バッチ項の列 (Batch column):", choices = NULL)))
    ),
    hr(),
    fluidRow(
      column(4, selectInput(ns("dim_reduction_method"), "可視化手法:", choices = c("MDS (edgeR)", "PCA", "t-SNE", "UMAP", "階層的クラスタリング (デンドログラム)" = "Dendrogram", "サンプル間距離ヒートマップ" = "DistHeatmap", "相関ヒートマップ" = "Heatmap", "ネットワークグラフ" = "Network"), selected = "PCA")),
      column(
        8,
        ...
        conditionalPanel(
          condition = paste0("input['", ns("dim_reduction_method"), "'] == 'PCA'"),
          fluidRow(
            column(6, numericInput(ns("pca_top_genes"), "使用遺伝子数(Top N変動):", 500, min = 50, step = 50)),
            column(3, selectInput(ns("pca_x"), "X軸:", choices = paste0("PC", 1:10), selected = "PC1")),
            column(3, selectInput(ns("pca_y"), "Y軸:", choices = paste0("PC", 1:10), selected = "PC2"))
          ),
          fluidRow(
            column(6, selectInput(ns("pca_color"), "色分け (Color):", choices = c("group"), selected = "group")),
            column(6, selectInput(ns("pca_shape"), "形分け (Shape):", choices = c("なし" = "none", "group"), selected = "none"))
          )
        ),
        ...
        conditionalPanel(
          condition = paste0("input['", ns("dim_reduction_method"), "'] == 'DistHeatmap'"),
          fluidRow(
            column(6, selectInput(ns("dist_heatmap_method"), "距離計算手法:", choices = c("euclidean", "maximum", "manhattan", "canberra", "minkowski"), selected = "euclidean")),
            column(6, selectInput(ns("dist_heatmap_hclust"), "クラスタリング:", choices = c("ward.D2", "ward.D", "complete", "average"), selected = "ward.D2"))
          ),
          checkboxInput(ns("dist_heatmap_show_values"), "距離の数値を表示", value = FALSE),
          helpText("サンプルとサンプルの間の距離(Distance)をヒートマップで可視化します。PCA同様に品質評価(QC)として重要です。")
        ),
        conditionalPanel(
          ...
          fluidRow(
            column(6, numericInput(ns("dim_fontsize_row"), "行(サンプル)フォントサイズ:", value = 8, min = 1, max = 50)),
            column(6, numericInput(ns("dim_fontsize_col"), "列(サンプル)フォントサイズ:", value = 8, min = 1, max = 50))
          ),
          checkboxInput(ns("dim_show_group"), "サンプル上のグループ名(カラーバー)を表示", value = TRUE)
        ),
...
dimensionReductionServer <- function(id, rv) {
  moduleServer(id, function(input, output, session) {
    ...
    observeEvent(rv$sample_metadata, {
      ...
      cols <- colnames(rv$sample_metadata)
      factor_cols <- setdiff(cols, c("id", "current_name", "active", "time"))
      updateSelectInput(session, "pca_color", choices = factor_cols, selected = "group")
      updateSelectInput(session, "pca_shape", choices = c("なし" = "none", factor_cols), selected = "none")
    ...
    prepared_data <- reactive({
      ...
      shiny::validate(shiny::need(length(keep_vector) == nrow(counts_df), "フィルタリング情報とカウントデータの行数が一致しません。フィルタリングタブを再実行してください。"))
      ...
      shiny::validate(shiny::need(nrow(filtered_counts_df) > 0, "フィルタリングの結果、遺伝子が残りませんでした。"))
      ...
      # バッチ補正の適用
      if (input$use_batch && !is.null(input$batch_col) && input$batch_col %in% colnames(samples_metadata_ordered)) {
        batch <- factor(samples_metadata_ordered[[input$batch_col]])
        if (nlevels(batch) >= 2) {
          # limma::removeBatchEffect を使用
          logcpm <- limma::removeBatchEffect(logcpm, batch = batch)
          message("[Dimension Reduction] Batch correction applied using column: ", input$batch_col)
        }
      }
      
      shiny::validate(shiny::need(nrow(logcpm) >= 2, "logCPM計算の対象となる遺伝子が少なすぎます。"))
    ...
    dimension_reduction_results <- reactive({
      ...
      y <- prep_data$y
      logcpm <- prep_data$logcpm
      group <- prep_data$group
      samples_metadata <- prep_data$samples_metadata
      n_samples <- ncol(y)
      plot_object <- NULL
      plot_type <- "other"
      title_suffix <- ""
      ...
      results <- tryCatch(
        {
          ...
          if (selected_method == "Dendrogram") {
            ...
            shiny::validate(shiny::need(n_samples >= 3, "クラスタリングには最低3サンプル必要です。"))
            ...
            if (selected_method == "MDS (edgeR)") {
              req(input$mds_top_genes)
              shiny::validate(shiny::need(n_samples >= 3, "MDSには最低3サンプル必要です。"))
              ...
              var_explained <- mds_results$var.explained
              x_lab <- paste0("Leading logFC dim 1 (", round(var_explained[1] * 100, 1), "%)")
              y_lab <- paste0("Leading logFC dim 2 (", round(var_explained[2] * 100, 1), "%)")
              title_suffix <- paste("(Top", input$mds_top_genes, "genes)")
            } else {
              top_genes_n <- switch(selected_method,
                "PCA" = input$pca_top_genes,
                "t-SNE" = input$tsne_top_genes,
                "UMAP" = input$umap_top_genes,
                500
              )
              req(top_genes_n)
              gene_vars <- matrixStats::rowVars(logcpm)
              n_select <- min(top_genes_n, nrow(logcpm))
              shiny::validate(shiny::need(n_select >= 2, "変動上位遺伝子が少なすぎます。"))
              select_genes <- order(gene_vars, decreasing = TRUE)[1:n_select]
              data_for_dim_red <- t(logcpm[select_genes, , drop = FALSE])
              title_suffix <- paste("(Top", n_select, "variable genes)")
              if (selected_method == "PCA") {
                shiny::validate(shiny::need(n_samples >= 3, "PCAには最低3サンプル必要です。"))
                pca_results <- prcomp(data_for_dim_red, scale. = TRUE)
                pca_summary <- summary(pca_results)

                pc_x_num <- as.integer(gsub("PC", "", input$pca_x))
                pc_y_num <- as.integer(gsub("PC", "", input$pca_y))
                max_pc <- ncol(pca_results$x)
                shiny::validate(shiny::need(pc_x_num <= max_pc && pc_y_num <= max_pc, paste0("サンプル数が少ないため、PC", pc_x_num, "やPC", pc_y_num, "は計算できません(Max: PC", max_pc, ")。")))

                var_explained_x <- pca_summary$importance[2, pc_x_num]
                var_explained_y <- pca_summary$importance[2, pc_y_num]

                plot_df <- data.frame(
                  Dim1 = pca_results$x[, pc_x_num],
                  Dim2 = pca_results$x[, pc_y_num],
                  Sample = rownames(pca_results$x),
                  Group = group[match(rownames(pca_results$x), colnames(y))],
                  stringsAsFactors = FALSE
                )

                # Add dynamic columns for color/shape
                factor_cols <- setdiff(colnames(samples_metadata), c("id", "current_name", "active", "time"))
                for (col in factor_cols) {
                  plot_df[[col]] <- samples_metadata[[col]][match(rownames(pca_results$x), samples_metadata$current_name)]
                }

                color_col <- input$pca_color
                shape_col <- input$pca_shape

                if (!is.null(color_col) && color_col %in% colnames(plot_df)) {
                  plot_df$PlotColor <- plot_df[[color_col]]
                } else {
                  plot_df$PlotColor <- plot_df$Group
                }

                if (!is.null(shape_col) && shape_col %in% colnames(plot_df)) {
                  plot_df$PlotShape <- as.character(plot_df[[shape_col]])
                } else if (!is.null(shape_col) && shape_col == "group") {
                  plot_df$PlotShape <- as.character(plot_df$Group)
                } else {
                  plot_df$PlotShape <- "1"
                }

                x_lab <- paste0("PC ", pc_x_num, " (", round(var_explained_x * 100, 1), "%)")
                y_lab <- paste0("PC ", pc_y_num, " (", round(var_explained_y * 100, 1), "%)")
              } else if (selected_method == "t-SNE") {
                min_samples_tsne <- 7
                shiny::validate(shiny::need(n_samples >= min_samples_tsne, paste0("t-SNEには最低", min_samples_tsne, "サンプル必要です。")))
                req(input$tsne_perplexity)
                input_perplexity <- input$tsne_perplexity
                max_valid_perplexity <- floor((n_samples - 1) / 3)
                min_valid_perplexity <- 2
                shiny::validate(shiny::need(input_perplexity >= min_valid_perplexity && input_perplexity <= max_valid_perplexity, paste0("Perplexityは", min_valid_perplexity, "-", max_valid_perplexity, "の範囲で設定してください。")))
                ...
                min_samples_umap <- 10
                shiny::validate(shiny::need(n_samples >= min_samples_umap, paste0("UMAPには最低", min_samples_umap, "サンプルを推奨します。")))
                req(input$umap_n_neighbors)
                umap_config <- umap::umap.defaults
                umap_config$n_neighbors <- input$umap_n_neighbors
                umap_config$n_neighbors <- min(umap_config$n_neighbors, n_samples - 1)
                shiny::validate(shiny::need(umap_config$n_neighbors >= 2, "近傍数(n_neighbors)が2未満です。"))
                umap_results <- umap::umap(data_for_dim_red, config = umap_config)
            ...
            if (selected_method == "PCA") {
              p <- ggplot(plot_df, aes(x = Dim1, y = Dim2, color = PlotColor, text = paste("Sample:", Sample, "<br>Color:", PlotColor, "<br>Shape:", PlotShape))) +
                {
                  if (input$pca_shape != "none") geom_point(aes(shape = PlotShape), size = 3, alpha = 0.8) else geom_point(size = 3, alpha = 0.8)
                } +
                labs(x = x_lab, y = y_lab, color = input$pca_color, shape = input$pca_shape) +
                theme_bw(base_size = 12) +
            ...
            plot_object <- p
            plot_object_reactive(p)
          } else if (selected_method == "DistHeatmap") {
            plot_type <- "static"
            req(input$dist_heatmap_method, input$dist_heatmap_hclust)
            shiny::validate(shiny::need(n_samples >= 3, "最低3サンプル必要です。"))
            dist_matrix <- as.matrix(dist(t(logcpm), method = input$dist_heatmap_method))
            annotation_col <- data.frame(Group = group, row.names = colnames(logcpm))
            factor_cols_for_anno <- setdiff(colnames(samples_metadata), c("current_name", "id", "active", "group", "time"))
            for (col in factor_cols_for_anno) {
              annotation_col[[col]] <- samples_metadata[[col]]
            }
            plot_object <- list(matrix = dist_matrix, annotation_col = annotation_col, clustering_method = input$dist_heatmap_hclust, cluster_cols = TRUE, cluster_rows = TRUE, display_numbers = input$dist_heatmap_show_values, dist_method = input$dist_heatmap_method, is_dist = TRUE)
            title_suffix <- paste("(Method:", input$dist_heatmap_method, ", Linkage:", input$dist_heatmap_hclust, ")")
          } else if (selected_method == "Heatmap") {
            plot_type <- "static"
            req(input$heatmap_dist_method, input$heatmap_hclust_method, input$cor_method)
            shiny::validate(shiny::need(n_samples >= 2, "相関計算には最低2サンプル必要です。"))
            ...
            shiny::validate(shiny::need(igraph::gsize(graph_obj) > 0, "指定した相関閾値ではサンプル間の繋がり（エッジ）がありません。"))
        ...
        error = function(e) {
          shiny::validate(paste(selected_method, "計算中にエラー:", e$message))
        }
      )
      results
    })
    ...
    output$staticPlot <- renderPlot({
      ...
      if (results$method == "Heatmap" || results$method == "DistHeatmap" || results$method == "Network") {
        if (results$method == "Heatmap" || results$method == "DistHeatmap") {
          req(is.list(plot_obj_data), all(c("matrix", "annotation_col") %in% names(plot_obj_data)))
          if (anyNA(plot_obj_data$matrix)) {
            shiny::validate("ヒートマップ描画エラー: 行列にNAが含まれています。")
          }

          if (isTRUE(plot_obj_data$is_dist)) {
            color_palette <- colorRampPalette(rev(RColorBrewer::brewer.pal(n = 9, name = "Blues")))(100)
          } else {
            color_palette <- colorRampPalette(rev(RColorBrewer::brewer.pal(n = 9, name = "RdYlBu")))(100)
          }

          f_row <- if (!is.null(input$dim_fontsize_row) && !is.na(input$dim_fontsize_row)) as.numeric(input$dim_fontsize_row) else 8
          f_col <- if (!is.null(input$dim_fontsize_col) && !is.na(input$dim_fontsize_col)) as.numeric(input$dim_fontsize_col) else 8
          show_grp <- if (!is.null(input$dim_show_group)) input$dim_show_group else TRUE

          ph <- pheatmap::pheatmap(plot_obj_data$matrix,
            ...
            main = plot_title, color = color_palette, na_col = "grey50", silent = TRUE
          )
          ...
          if (length(unique_groups) > 0) {
            num_colors <- length(unique_groups)
            if (num_colors <= 9) {
              group_colors <- RColorBrewer::brewer.pal(max(3, num_colors), "Set1")[1:num_colors]
            } else {
              group_colors <- rainbow(num_colors)
            }
